...
function isErrorLike(error) {
  return error instanceof Object && "message" in error && typeof (error === null || error === void 0 ? void 0 : error.message) === "string";
}
...
function shouldRejectError(error) {
  ...
  if (isErrorLike(error)) {
    if (isEthersRequestErrorLike(error)) {
      var method = JSON.parse(error.requestBody).method;
    ...
    if (swapErrorToUserReadableMessage.didUserReject(error)) return true;
    ...
    if (error instanceof DOMException && (error === null || error === void 0 ? void 0 : error.name) === "AbortError") return true;
  }
  return false;
}
