...
var moonpay = require('../../../../assets/svg/moonpay.cjs');
...
var COMMON_CONTRACTS = _defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"]({}, sdkCore.UNI_ADDRESSES[sdkCore.ChainId.MAINNET].toLowerCase(), {
  title: index.i18n._(
  /*i18n*/
  {
    id: "r6fWDd",
...
function callsPositionManagerContract(assetActivity) {
  var supportedChain = util.supportedChainIdFromGQLChain(assetActivity.chain);
  if (!supportedChain) return false;
  return addresses.isSameAddress(assetActivity.details.to, sdkCore.NONFUNGIBLE_POSITION_MANAGER_ADDRESSES[supportedChain]);
}
...
function getSwapTitle(sent, received) {
  var supportedSentChain = util.supportedChainIdFromGQLChain(sent.asset.chain);
  var supportedReceivedChain = util.supportedChainIdFromGQLChain(received.asset.chain);
  ...
  if (sent.tokenStandard === "NATIVE" && addresses.isSameAddress(tokens.nativeOnChain(supportedSentChain).wrapped.address, received.asset.address)) return index.i18n._(
  /*i18n*/
  {
    id: "P+Mxf/",
    message: "Wrapped"
  });else if (received.tokenStandard === "NATIVE" && addresses.isSameAddress(tokens.nativeOnChain(supportedReceivedChain).wrapped.address, received.asset.address)) {
    return index.i18n._(
    /*i18n*/
    {
      id: "sNANgM",
...
function getTransactedValue(transactedValue) {
  ...
  var price = (transactedValue === null || transactedValue === void 0 ? void 0 : transactedValue.currency) === typesAndHooks.Currency.Usd ? (_transactedValue$valu = transactedValue.value) !== null && _transactedValue$valu !== void 0 ? _transactedValue$valu : undefined : undefined;
  return price;
}
function parseSwap(changes, formatNumberOrString) {
  if (changes.NftTransfer.length > 0 && changes.TokenTransfer.length === 1) {
    ...
    var title = changes.NftTransfer[0].direction === "IN" ? index.i18n._(
    /*i18n*/
    {
      id: "8DIDYI",
      message: "Bought"
  ...
  if (changes.TokenTransfer.length >= 2) {
    ...
    if (sent && received) {
      ...
      return {
        ...
        currencies: [util.gqlToCurrency(sent.asset), util.gqlToCurrency(received.asset)]
      };
    }
  }
  return {
...
function parseLend(changes, formatNumberOrString) {
  ...
  if (_native && erc20 && ((_gqlToCurrency = util.gqlToCurrency(_native)) === null || _gqlToCurrency === void 0 ? void 0 : _gqlToCurrency.wrapped.address) === ((_gqlToCurrency2 = util.gqlToCurrency(erc20)) === null || _gqlToCurrency2 === void 0 ? void 0 : _gqlToCurrency2.wrapped.address)) {
...
function parseApprove(changes) {
  if (changes.TokenApproval.length === 1) {
    var title = parseInt(changes.TokenApproval[0].quantity) === 0 ? index.i18n._(
    /*i18n*/
    {
      id: "qFn8Ux",
    ...
    var currencies = [util.gqlToCurrency(changes.TokenApproval[0].asset)];
    return {
      title: title,
      descriptor: descriptor,
...
function parseLPTransfers(changes, formatNumberOrString) {
  ...
  return {
    ...
    currencies: [util.gqlToCurrency(poolTokenA.asset), util.gqlToCurrency(poolTokenB.asset)]
  };
}
function parseSendReceive(changes, formatNumberOrString, assetActivity) {
  ...
  if (changes.NftTransfer.length === 1) {
    ...
    currencies = [util.gqlToCurrency(transfer.asset)];
  }
  if (transfer && assetName && amount) {
    var isMoonpayPurchase = constants.MOONPAY_SENDER_ADDRESSES.some(function (address) {
      var _transfer;
      return addresses.isSameAddress(address, (_transfer = transfer) === null || _transfer === void 0 ? void 0 : _transfer.sender);
    });
    if (transfer.direction === "IN") {
      return isMoonpayPurchase && transfer.__typename === "TokenTransfer" ? {
        ...
        descriptor: "".concat(amount, " ").concat(assetName, " ").concat(index.i18n._(
        /*i18n*/
        {
          id: "sQv06Y",
          message: "for"
        ...
        otherAccount: addresses.isAddress(transfer.sender) || undefined,
        currencies: currencies
      };
    } else {
      return {
        ...
        otherAccount: addresses.isAddress(transfer.recipient) || undefined,
        currencies: currencies
      };
    }
  }
...
var ActivityParserByType = _defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"]({}, typesAndHooks.ActivityType.Swap, parseSwap), typesAndHooks.ActivityType.Lend, parseLend), typesAndHooks.ActivityType.SwapOrder, parseSwapOrder), typesAndHooks.ActivityType.Approve, parseApprove), typesAndHooks.ActivityType.Send, parseSendReceive), typesAndHooks.ActivityType.Receive, parseSendReceive), typesAndHooks.ActivityType.Mint, parseMint), typesAndHooks.ActivityType.Unknown, parseUnknown);
...
function parseUniswapXOrder(_ref4) {
  ...
  if (details.orderStatus === typesAndHooks.SwapOrderStatus.Open) return undefined;
  ...
  var uniswapXOrderStatus = constants.OrderStatusTable[orderStatus];
  var _OrderTextTable$unisw = constants.OrderTextTable[uniswapXOrderStatus],
  ...
  var supportedChain = util.supportedChainIdFromGQLChain(chain);
  if (!supportedChain) {
    util.logSentryErrorForUnsupportedChain({
      extras: {
        details: details
  ...
  return {
    ...
    currencies: [util.gqlToCurrency(inputToken), util.gqlToCurrency(outputToken)],
    title: title,
    descriptor: descriptor,
...
function parseRemoteActivity(assetActivity, formatNumberOrString) {
  try {
    ...
    var defaultFields = _defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"](_defineProperty__default["default"]({
      // @ts-ignore
...
var getTimeSince = function getTimeSince(timestamp) {
  ...
  if ((interval = seconds / ms__default["default"]("1y")) > 1) return Math.floor(interval) + "y";
  if ((interval = seconds / ms__default["default"]("30d")) > 1) return Math.floor(interval) + "mo";
  if ((interval = seconds / ms__default["default"]("1d")) > 1) return Math.floor(interval) + "d";
  if ((interval = seconds / ms__default["default"]("1h")) > 1) return Math.floor(interval) + "h";
  if ((interval = seconds / ms__default["default"]("1m")) > 1) return Math.floor(interval) + "m";else return Math.floor(seconds / ms__default["default"]("1s")) + "s";
};
...
function useTimeSince(timestamp) {
  var _useState = React.useState(getTimeSince(timestamp)),
  ...
  React.useEffect(function () {
    var refreshTime = function refreshTime() {
      return setTimeout(function () {
        if (Math.floor(Date.now() - timestamp * 1000) / ms__default["default"]("61s") <= 1) {
          setTimeSince(getTimeSince(timestamp));
          timeout = refreshTime();
        }
      }, ms__default["default"]("1s"));
    };
...
exports.parseRemoteActivities = parseRemoteActivities;
